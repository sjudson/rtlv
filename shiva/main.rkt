          ...
          (let ()
            (define any-failed
              (for/list ([v value])
                ...
                (if (@unsat? r) #f r)))
            (if (andmap not any-failed) ; all #f
                (@unsat)
                any-failed))))
          ...
          (define+time (res state-analysis-time)
            (if (>= cycle try-verify-after)
                (let ()
                  ...
                  (define any-failed
                    (for/list ([name-value states])
                      (match-define (cons name value) name-value)
                      (define r (only-depends-on* value allowed-dependencies))
                      (if (@unsat? r) '() (list name value r))))
                  (if (ormap empty? any-failed)
                      (let ()
                        (displayln " --> unsat!")
                        (@unsat))
                      (let ()
                        (match-define (list name value r) (car any-failed))
                        (cond
                          [(@unknown? r)
